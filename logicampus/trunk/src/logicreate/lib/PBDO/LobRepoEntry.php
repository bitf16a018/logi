...
class LobRepoEntryBase {
	...
	/**
	 * Retrieves an array of lob_test_qst objects via the foreign key image_content_id.
	 * 
	 * @param String $dsn the name of the data source to use for the sql query.
	 * @return Array related objects.
	 */
	function getLobTestQstsByImageContentId($dsn='default') {
		if ( $this->lobRepoEntryId == '' ) { trigger_error('Peer doSelect with empty key'); return false; }
		$array = LobTestQstPeer::doSelect('image_content_id = \''.$this->lobRepoEntryId.'\'',$dsn);
		return $array;
	}
