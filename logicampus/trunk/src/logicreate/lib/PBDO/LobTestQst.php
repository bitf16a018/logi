...
class LobTestQstBase {
	...
	/**
	 * Retrieves one lob_repo_entry object via the foreign key image_content_id.
	 * 
	 * @param String $dsn the name of the data source to use for the sql query.
	 * @return Object the related object.
	 */
	function getLobRepoEntryByImageContentId($dsn='default') {
		if ( $this->imageContentId == '' ) { trigger_error('Peer doSelect with empty key'); return false; }
		$array = LobRepoEntryPeer::doSelect('lob_repo_entry_id = \''.$this->imageContentId.'\'',$dsn);
		if ( count($array) > 1 ) { trigger_error('multiple objects on one-to-one relationship'); }
		return $array[0];
	}
...
class LobTestQstPeerBase {
	...
	function doSelect($where,$dsn="default") {
		...
		$st->fields['image_content_id'] = 'image_content_id';
	...
	function doInsert(&$obj,$dsn="default") {
		...
		$st->fields['image_content_id'] = $obj->imageContentId;
		...
		$st->nulls['image_content_id'] = 'image_content_id';
	...
	function row2Obj($row) {
		...
		$x->imageContentId = $row['image_content_id'];
